...
def find_path_to_setup_from_repo_root(location, repo_root):
    """
    Find the path to `aa.py` by searching up the filesystem from `location`.
    Return the path to `aa.py` relative to `repo_root`.
    Return None if `aa.py` is in `repo_root` or cannot be found.
    """
    # find aa.py
    orig_location = location
    while not os.path.exists(os.path.join(location, 'aa.py')):
        ...
        if location == last_location:
            ...
            # finding aa.py
            logger.warning(
                "Could not find aa.py for directory %s (tried all "
                "parent directories)",
                orig_location,
            )
            return None
...
class VersionControl(object):
    ...
    def get_subdirectory(cls, location):
        """
        Return the path to aa.py, relative to the repo root.
        Return None if aa.py is in the repo root.
        """
        return None
